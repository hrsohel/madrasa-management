...
export interface IAddress extends Document {
  // Present Address
  presentDivision: typeof DIVISIONS[number];
  presentDistrict: string;
  presentUpazila: string;
  presentUnion: string;
  presentVillage: string;
  presentOthers?: string; // e.g., Road no, House no, Holding no

  // Permanent Address
  permanentDivision: typeof DIVISIONS[number];
  permanentDistrict: string;
  permanentUpazila: string;
  permanentUnion: string;
  permanentVillage: string;
  permanentOthers?: string;

  // Link to Student (1-to-1 relationship)
  student: Schema.Types.ObjectId; // ref: 'Student'

  // Optional: shortcut flag
